...
class TodoStarIcon extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = {
      star_starred: this.props.todo.starred
    };
  ...
  handleClickIcon() {
    const original = JSON.parse(JSON.stringify(this.props.todo));
    ...
    this.replaceTodo(original, this.props.todo);
  }
  ...
  render() {
    const iconClass = this.state.star_starred ? (this.props.todo.completed ? 'starred starred_completed' : 'starred') : 'star';

    return (
      <span className={iconClass}
            onClick={this.handleClickIcon}
            dangerouslySetInnerHTML={{
              __html: this.state.star_starred
